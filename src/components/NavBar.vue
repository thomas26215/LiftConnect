<template>
  <nav :class="{ scrolled, 'menu-open': menuOpen }">
    <div class="scroll-bar" :style="{ transform: `scaleX(${scrollProgress})` }"></div>

    <div class="nav-inner" :class="{ mounted }">
      <!-- Logo -->
      <a href="#" class="logo" @mouseenter="logoHover = true" @mouseleave="logoHover = false">
      ...
      <!-- Right -->
      <div class="nav-right">
        <a href="#" class="login" :style="{ '--i': 2 }" :class="{ mounted }">
          Se connecter
        </a>
        <a
          href="#download"
        ...
        >
          <span class="cta-inner">
            <span class="cta-text">Telecharger</span>
    ...
    <Transition name="mobile">
      <div v-if="menuOpen" class="mobile-menu">
        ...
        >
          ...
          <span v-if="link.arrow" class="m-arrow">&#8599;</span>
        </a>
      </div>
    </Transition>
  </nav>
...
const mobileItems = [
  { href: '#features', label: 'Fonctionnalites', cls: 'mobile-link' },
  { href: '#download', label: 'Telecharger', cls: 'mobile-link' },
  { href: '#', label: 'Se connecter', cls: 'mobile-link mobile-login' },
  { href: '#download', label: 'Telecharger', cls: 'mobile-cta', arrow: true },
]
...
function updatePill(idx) {
  ...
  const lis = linksRef.value.querySelectorAll('li')
  const li = lis[idx]
  if (!li) return
...
.logo:hover .logo-icon {
  ...
  border-color: rgba(186, 242, 216, 0.45);
}
...
.cta:hover {
  ...
  box-shadow: 0 6px 24px rgba(186, 242, 216, 0.4);
}
.cta.clicked {
...
.mobile-cta {
  ...
  background: rgba(186, 242, 216, 0.07) !important;
  border: 1px solid rgba(186, 242, 216, 0.15) !important;
}
.m-arrow {
  opacity: 0.6;
  font-size: 0.75rem;
...
.mobile-enter-active {
  animation: menuSlide 0.35s cubic-bezier(0.4, 0, 0.2, 1);
}
.mobile-leave-active {
  animation: menuSlide 0.25s cubic-bezier(0.4, 0, 0.2, 1) reverse;
}
@keyframes menuSlide {

  ...
  <nav :class="{ scrolled, 'menu-open': menuOpen }">
    ...
    <div class="nav-inner" :class="{ mounted }">
      ...
      <div class="nav-right">
        ...
        <!-- Extra links (Confidentialité, Supprimer mon compte…) -->
        <template v-if="extraLinks.length">
          <component
            ...
            :to="isAnchor(link.href) ? undefined : link.href"
            class="extra-link"
            :class="[link.variant ? `extra-link--${link.variant}` : '', { mounted }]"
            :style="{ '--i': i }"
          >
            {{ link.label }}
          </component>
          <span class="nav-divider" v-if="showLogin || showCta || backLink"></span>
        </template>
...
const props = defineProps({
  ...
  // [{ href, label, variant?: 'danger' | 'muted' }]
...
const mobileItems = computed(() => {
  ...
  props.extraLinks.forEach(l => items.push({
    href: l.href, label: l.label,
    cls: l.variant === 'danger'
      ? 'mobile-link mobile-link--danger'
      : 'mobile-link mobile-link--muted',
  }))
...
function spawnParticles() {
  ...
  particles.value = Array.from({ length: count }, (_, i) => {
    ...
    return { id: Date.now() + i, style: { '--px': Math.cos((angle * Math.PI) / 180) * dist + 'px', '--py': Math.sin((angle * Math.PI) / 180) * dist + 'px', '--delay': i * 20 + 'ms' } }
  })
...
.logo-text { display: inline-block; transition: letter-spacing 0.3s; }
...
.extra-link {
  ...
  padding: 5px 11px; border-radius: 9px; border: 1px solid transparent;
  opacity: 0; transform: translateY(-6px);
  animation: fadeSlideDown 0.4s cubic-bezier(0.4,0,0.2,1) forwards;
  animation-delay: calc(var(--i) * 60ms + 250ms); animation-play-state: paused;
...
/* Back */
.back-btn { display: inline-flex; align-items: center; gap: 6px; font-size: 0.78rem; font-weight: 600; color: rgba(255,255,255,0.58); text-decoration: none; padding: 6px 12px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08); background: rgba(255,255,255,0.03); opacity: 0; transform: translateY(-6px); animation: fadeSlideDown 0.4s cubic-bezier(0.4,0,0.2,1) 250ms forwards; animation-play-state: paused; transition: color 0.2s, border-color 0.2s, background 0.2s, transform 0.2s cubic-bezier(0.34,1.56,0.64,1); }
.back-btn.mounted { animation-play-state: running; }
.back-btn:hover { color: rgba(186,242,216,0.9); border-color: rgba(186,242,216,0.2); background: rgba(186,242,216,0.05); transform: translateX(-2px); }
.back-arrow { font-size: 0.85rem; }

/* Login */
.login { font-size: 0.8rem; font-weight: 600; color: rgba(255,255,255,0.58); text-decoration: none; padding: 6px 10px; opacity: 0; transform: translateY(-6px); animation: fadeSlideDown 0.4s cubic-bezier(0.4,0,0.2,1) forwards; animation-delay: calc(var(--i) * 80ms + 200ms); animation-play-state: paused; transition: color 0.2s, transform 0.2s cubic-bezier(0.34,1.56,0.64,1); }
